<template>
    <div class="ww-popup-wordpress-sync">
        <button class="wordpress-sync__all ww-editor-button -primary -green" @click="syncAll" :disabled="isFetching">
            <div v-if="!isFetching">Synchronize all</div>
            <div v-else>Fetching...</div>
        </button>
        <div class="wordpress-sync__row" v-for="(query, index) in settings.privateData.queries" :key="index">
        ...
        <div class="wordpress-sync__separator"></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async sync(query) {
            ...
            await wwLib.wwPlugins.pluginWordpress.sync(query);
